...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      const targets = [];
      const step = density <= 1 ? 1 : Math.max(2, Math.floor(density));
      ...
      const stride =
        density <= 1 ? 1 : Math.max(1, Math.ceil(targets.length / maxParticles));
      ...
      particles = selected.map((target, index) => {
        ...
        return {
          ...
          targetX: target.x,
          targetY: target.y,
          size: Math.max(1.2, particleSize * (0.9 + target.alpha * 0.35)),
          color: particleColor,
          seed,
          depth,
          delay: seed * stagger
